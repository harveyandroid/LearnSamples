...
public class GenericTest {
    void test() {
        List<? extends Self> a = new ArrayList<>();//参数类型下界是Self
//        a.add(new Son());//error 不能放入任何类型，因为编译器只知道a中应该放入Self的某个父类，但具体放哪种父类它并不知道，因此，除了null以外，不能放入任何类型
...
//        Son s3 = a.get(0); //error:子类不能接收父类型参数
        ...
        List<? super Self> b = new ArrayList<>();//参数类型上界是Self
