...
package com.harvey.openat.driver;
...
/**
 * Driver for CH340, maybe also working with CH341, but not tested See
...
public class Ch34xSerialDriver implements UsbSerialDriver {
	...
	public static Map<Integer, int[]> getSupportedDevices() {
		...
		supportedDevices.put(UsbId.VENDOR_QINHENG, new int[]{UsbId.QINHENG_HL340});
	...
	public class Ch340SerialPort extends CommonUsbSerialPort {
		...
		public void open(UsbDeviceConnection connection) throws IOException {
			...
			try {
				if (mDevice.getInterfaceCount() == 0) {
					throw new IOException("Interface Count is null");
				}
		...
		private int controlOut(int request, int value, int index) {
			...
			return mConnection.controlTransfer(REQTYPE_HOST_TO_DEVICE, request, value, index, null, 0,
					USB_TIMEOUT_MILLIS);
		}

		private int controlIn(int request, int value, int index, byte[] buffer) {
			final int REQTYPE_HOST_TO_DEVICE = UsbConstants.USB_TYPE_VENDOR | UsbConstants.USB_DIR_IN;
			return mConnection.controlTransfer(REQTYPE_HOST_TO_DEVICE, request, value, index, buffer, buffer.length,
					USB_TIMEOUT_MILLIS);
		}
		...
		private void checkState(String msg, int request, int value, int[] expected) throws IOException {
			...
			for (int i = 0; i < expected.length; i++) {
				...
				if (expected[i] != current) {
					throw new IOException("Expected 0x" + Integer.toHexString(expected[i]) + " bytes, but get 0x"
							+ Integer.toHexString(current) + " [" + msg + "]");
				}
			}
		}
		...
		private void setBaudRate(int baudRate) throws IOException {
			int[] baud = new int[]{2400, 0xd901, 0x0038, 4800, 0x6402, 0x001f, 9600, 0xb202, 0x0013, 19200, 0xd902,
					0x000d, 38400, 0x6403, 0x000a, 115200, 0xcc03, 0x0008};
